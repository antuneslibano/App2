...
export type TabId = 'mine' | 'shop' | 'upgrades' | 'gems' | 'prestige';
      ...
      {TABS.map((tab) => {
        ...
        return (
          <Pressable key={tab.id} style={styles.tab} onPress={() => onChange(tab.id)}>
            <GameIcon name={tab.icon} size={20} color={isActive ? theme.accent : theme.textDim} />
